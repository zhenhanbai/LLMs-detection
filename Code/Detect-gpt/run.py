...
def tokenize_and_mask(text, span_length, pct, ceil_pct=False):
    """
    通过在文本中随机插入掩码（mask）来生成新的文本。
    函数的参数包括文本（text）、跨度长度（span_length）、掩码百分比（pct）以及一个可选的布尔参数 ceil_pct。
    """
    tokens = text.split(' ')
    mask_string = '<<<mask>>>'

    n_spans = pct * len(tokens) / (span_length + args.buffer_size * 2) # 文本中插入掩码时需要考虑的周围单词的数量
    if ceil_pct:
    ...
    while n_masks < n_spans:
        # 保证不越界，并把start:end 的token变成'<<<mask>>>'
...
def replace_masks(texts):
    ...
    stop_id = mask_tokenizer.encode(f"<extra_id_{max(n_expected)}>")[0]  #文本结束的ID
...
"""
将文本扰动应用到输入的文本列表中，并通过分批次的方式处理大型文本集
"""
...
def save_roc_curves(experiments):
    ...
    plt.savefig(f"{SAVE_FOLDER}/roc_curves.png", dpi=300)
...
def save_ll_histograms(experiments):
    ...
    for experiment in experiments:
        try:
            ...
            plt.ylabel('count')
            plt.legend(loc='upper right')
            plt.savefig(f"{SAVE_FOLDER}/ll_histograms_{experiment['name']}.png", dpi=300)
        except:
            pass
...
def save_llr_histograms(experiments):
    ...
    for experiment in experiments:
        try:
            ...
            plt.ylabel('count')
            plt.legend(loc='upper right')
            plt.savefig(f"{SAVE_FOLDER}/llr_histograms_{experiment['name']}.png", dpi=300)
        except:
            pass
...
def generate_data(dataset, key):
    ...
    修改成我们的数据集
    """
    if dataset in["Hello-SimpleAI/HC3"]:
        data_old = datasets.load_dataset(dataset, name="all", split='train',cache_dir=cache_dir)[key]
        data = [item for sublist in data_old for item in sublist]
    # elif dataset in ["shuffled_zh_sent_test"]:
    #     data = pd.read_csv("../../data/shuffled_zh_sent_test.csv")
    #     data = data[data['label']==0]
    #     data = Dataset.from_pandas(data)['answer']
    ...
    if dataset in ['writing', 'squad', 'xsum', 'Hello-SimpleAI/HC3']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default="Hello-SimpleAI/HC3")
    parser.add_argument('--dataset_key', type=str, default="chatgpt_answers")
